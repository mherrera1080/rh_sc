...
$(document).on("click", ".btnFacturaEditar", function () {
  ...
  $.ajax({
    url: `${base_url}/SolicitudFondos/getFacturaServicio/${factura}`,
    method: "GET",
    dataType: "json",
    success: function (response) {
      if (response.status) {
        ...
        $("#kilometraje").val(response.data.kilometraje);
        $("#estado").val(response.data.estado);
        $("#tipo_servicio").val(response.data.tipo_servicio);
        $("#ln").val(response.data.ln);
        $("#usuario").val(response.data.usuario);
        $("#codigo_ax").val(response.data.codigo_ax);
        $("#tipo_mantenimiento").val(response.data.tipo_mantenimiento);

        const materiales = response.data.materiales_array;
...
document.addEventListener("submit", function (event) {
  if (event.target && event.target.id === "ServicioEdit") {
    event.preventDefault();
    let formData = new FormData(event.target);
    let ajaxUrl = base_url + "/SolicitudFondos/updateDetalleServicio";
